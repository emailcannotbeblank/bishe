...
class MultimodalPretrainingDataset(data.Dataset):
    def __init__(self, cfg, split="train", transform=None):
        ...
        # zry changes
        self.transform = self.get_transformers()
        self.max_word_num = self.cfg.data.text.captions_per_image

        # read CheXpert csv file
        csv_path = CHEXPERT_MASTER_CSV
        # csv_path = "train_100_small.csv"
        self.df = pd.read_csv(csv_path)
        # zry change
        self.df[CHEXPERT_PATH_COL] = self.df[CHEXPERT_PATH_COL].apply(
            lambda x: os.path.join(CHEXPERT_DATA_DIR, "/".join(x.split("/")[0:]))
        )
    ...
    def __getitem__(self, index):
        ...
        imgs1,imgs2 = self.get_imgs(key, self.transform)

        # randomly select a sentence
        caps, cap_len, sent = self.get_caption(key) 
        
        # zry changes
        return imgs1,imgs2, sent
    ...
    def get_transformers(self):
        ...
        pretrain_transform = transforms.Compose(t)
