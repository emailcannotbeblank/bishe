...
# zry add
from .models.vit import VisionTransformer,PretrainedVit
...
def load_img_classification_model(
    ...
    # zry add
    if name == "vit":
        vit_encoder = VisionTransformer().to(device)
        img_model = PretrainedVit(vit_encoder, num_cls, 768)
        return img_model
    elif name == "vit_pretrained":
        vit_encoder = load_pretrained_vit()
        img_model = PretrainedVit(vit_encoder, num_cls, 768)
        return img_model
    elif name == "resnet_random":
        from omegaconf import OmegaConf
        cfg = OmegaConf.load("./configs/pneumonia_classification_config.yaml")
        cfg.model.vision.model_name = "resnet_50"
        image_encoder = ImageEncoder(cfg)
        img_model = PretrainedImageClassifier(image_encoder, num_cls, 2048)
        return img_model
...
def load_pretrained_vit():
    ...
    checkpoint = torch.load("./output/pretrain/checkpoint_29.pth", map_location='cpu')
    state_dict = checkpoint['model']
    model.load_state_dict(state_dict)
    vit = copy.deepcopy(model.visual_encoder)
    del model
    return vit
