...
# zry changes
from models.ot import optimal_transport_dist
...
class ALBEF(nn.Module):
    ...
    def forward(self, image, image_aug, text, alpha=0):
        ...
        # zry changes
        image = nn.Upsample(size=(256, 256), mode="bilinear", align_corners=True)(image)
        image_aug = nn.Upsample(size=(256, 256), mode="bilinear", align_corners=True)(image_aug)
        ...
        # zry: ot loss
        loss_ot = self.ot_loss(image_embeds,text_embeds,text)
        ...
        loss_ita = (loss_i2i_inMod_l+loss_i2t+loss_t2i+loss_i2i)/4
        ...
        return loss_mlm, loss_ita, loss_itm, loss_ot
    ...
    def ot_loss(self,image_embeds,text_embeds,text):
        B = image_embeds.shape[0]
        N = image_embeds.shape[1]
        txt_pad = 1 - text.attention_mask
        # txt_pad = txt_pad.int()
        # img_pad = torch.zeros((B,N), device = txt_pad.device,dtype = torch.int32)
        txt_pad = txt_pad.bool()
        img_pad = torch.zeros((B,N), device = txt_pad.device,dtype = torch.bool)
        
        loss = optimal_transport_dist(text_embeds,image_embeds,txt_pad,img_pad)
        return loss.mean()
