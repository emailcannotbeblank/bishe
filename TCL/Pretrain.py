...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i, (image, image_aug, text) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # import pdb
        # pdb.set_trace()
        loss_mlm, loss_ita, loss_itm, loss_ot = model(image, image_aug, text_input, alpha = alpha)  
            
        loss = loss_mlm * 0.2 + loss_ita + loss_itm + loss_ot   
        ...
        metric_logger.update(loss_ot=loss_ot.item())
...
def main(args, config):
    ...
    dist.init_process_group('nccl', init_method='file:///home/zry/temp/TCL', rank=0, world_size=1)
